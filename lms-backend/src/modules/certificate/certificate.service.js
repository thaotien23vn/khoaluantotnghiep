...
class CertificateService {
   ...
   */
  async generateCertificate(userId, courseId, res) {
  ...
  _drawCertificateDesign(doc, studentName, courseTitle, certificateId, completionDate) {
    ...
    doc.font('Helvetica-Bold')
       .fontSize(35)
       .fillColor('#2d3748')
       .text(this._removeVietnameseTones(studentName).toUpperCase(), 0, 260, { align: 'center' }); // Simple accent removal because PDFKit default fonts don't support unicode Vietnamese perfectly
    ...
    doc.font('Helvetica-Bold')
       .fontSize(25)
       .fillColor('#1a365d')
       .text(this._removeVietnameseTones(courseTitle), 30, 360, { align: 'center', width: width - 60 });
    ...
    // Signature
    doc.text('T-LMS Platform', width - 300, 480, { align: 'right' });
